...
export async function GET(context: APIContext) {
  ...
  const posts = locales.flatMap((locale) => selectPosts(entries, locale));

  const body = llmsTxt({
    site,
    summary: `${profile.role['pt-BR']}. ${profile.tagline['pt-BR']}. Site pessoal em português e inglês, com perfil profissional e blog técnico.`,
    sections: [
      {
        heading: 'Páginas',
        entries: [
          ...
          },
          {
            title: 'Blog (pt-BR)',
            path: '/blog/',
            description: 'Índice dos posts, agrupado por ano e mês. Mesmo índice em /en/blog/.',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        entries: [
          ...
          { title: 'RSS (pt-BR)', path: '/rss.xml', description: 'Feed dos posts em português.' },
          { title: 'RSS (en)', path: '/en/rss.xml', description: 'Feed dos posts em inglês.' },
        ],
      },
    ],
    notes: [
      'Todo o conteúdo é escrito por Henrique Artur.',
      'As páginas trazem dados estruturados schema.org (Person, ProfilePage, BlogPosting, BreadcrumbList) — costumam ser mais confiáveis que extrair do HTML.',
